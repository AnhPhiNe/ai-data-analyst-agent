...
- **Agent orchestration:** guardrails, deterministic router, optional Gemini/Groq planner fallback, validation, single-tool execution, response composition.
...
- **Bounded planner repair:** invalid LLM-selected tool calls can be retried once with validation feedback before asking the user to clarify.
...
The dashboard uses deterministic chart recommendation so repeated uploads of the same dataset produce the same chart types and ordering. The configured LLM is used for chat planning and optional suggested content, not for changing the dashboard chart set.
