...
- **Bounded planner repair:** invalid Gemini-selected tool calls can be retried once with validation feedback before asking the user to clarify.
...
flowchart TD
    ...
    L --> V
    V -->|invalid Gemini call| PR[Planner Retry With Validation Feedback]
    PR --> V
...
`MAX_PLANNER_VALIDATION_RETRIES` controls bounded Gemini planner correction after an invalid tool call. The default is `1`.
...
- The current orchestrator is constrained single-turn tool calling with bounded validation repair; autonomous ReAct multi-step planning is a roadmap item.
